...
//
// ARCHITECTURE (0.6.0): STATIC COMPOSITION, ZERO RUNTIME ROW OWNERSHIP.
//
// The bind and the fence are decided ONCE PER BOOT by the bundle patch
// (cordis.patch.yml). Its `webserver`/`connection` overrides carry loader
// `!!js` expressions that read the `lan-access` entry's disabled state —
// the exact state dshmarket's toggle writes as `disabled: true` before
// activation — so:
//
//   - plugin enabled  -> webserver binds `0.0.0.0`, the fence extends with
//     the configured `accessHosts` + the deployment-derived
//     `webRuntime.trustedHosts`, and the configured cookie lifetime
//     applies;
//   - plugin disabled -> both expressions return the official expressions
//     (`ctx.webStartup.host` / `ctx.webRuntime.trustedHosts`), i.e. the
//     plain dsh behavior, and no official row is ever touched by a toggle.
//
// This module performs NO row updates at runtime: no `entry.update`, no
// fiber restarts, no boot-time rewrites (the 0.5.x mechanism that wedged
// the webserver listener whenever a toggle or a market boot replay flipped
// the rows inside a live process). Toggling the plugin — in the settings
// tab or in the market — only starts/stops this fiber; the binding changes
// on the next restart, which is exactly what the settings tab promises.
// The market's hot toggle is therefore safe: nothing in this module reacts
// to it.
//
// What this module still owns at runtime:
//   - the settings namespace (`lan-access`) the settings tab edits;
//   - the Remote (`lanAccess/overview`) reporting live vs configured state
//     (including the "restart to apply" pending case);
//   - the optional noAuth prototype patch (no rows touched, safe);
//   - the boot-time wiring report + the LAN URL line.
//
// Upgrade resilience: the overrides are patches by id; a missing row or a
// changed official schema wins (the expression falls back to the official
// defaults on any error), and the boot report logs the drift instead of
// silently pretending.
//
...
export function settingsSnapshot(values) {
	...
	return Object.freeze({
		...
		noAuth,
		/** Browser-session cookie lifetime in days (applied by the bundle patch expression at boot). */
		sessionDays,
		/** webserver bind expected by the bundle patch: 0.0.0.0 while enabled, loopback while disabled. */
 ...
 * configured values (read fresh from the settings scope on every call, so
 * a save shows its pending state immediately), the effective bind/trust
 * state, wiring verification, and the LAN candidate list. The Typert
 * Gateway discovers it by the `typertRemote` binding + method markers below
 * — no Service base needed. The method must live on the PROTOTYPE (the
 * gateway reads the method from the prototype chain), so the object is
 * created from a shared prototype.
 * @param ctx - the plugin context (for `ctx.get` lookups).
 * @param scope - the live settings scope (re-read per call).
 * @param wiring - mutable wiring-verification record.
 * @returns the Remote value to provide.
 */
function createLanAccessRemote(ctx, scope, wiring) {
	const prototype = {
		...
		overview() {
			...
			// Wired = the configured state is what the live process serves.
			// With static composition the bind can only change on restart,
			// so "enabled but not wired" is the pending-restart case the
			// settings tab already describes.
			const wiredNow = snapshot.enabled && bindHost === "0.0.0.0" && snapshot.trustedHosts.every((host) => trustedHosts.includes(host));
			wiring.webserver = bindHost === "0.0.0.0";
			wiring.connection = snapshot.trustedHosts.every((host) => trustedHosts.includes(host));
			wiring.verified = wiredNow;
			const lanUrls = [];
			if (wiredNow && port !== void 0) {
...
/**
 * Mount the plugin: register the settings namespace, expose the Remote,
 * and — after the tree settles — report the wiring. No row is ever
 * updated here: the bind and the fence are decided by the bundle patch's
 * `!!js` expressions at boot (see cordis.patch.yml), so this fiber's
 * lifecycle (including a dshmarket hot disable) cannot disturb the
 * webserver listener.
...
export function apply(ctx) {
	...
	const boot = settingsSnapshot(scope.get());
	const wiring = {
		webserver: false,
		connection: false,
		verified: false
	};
	ctx.provide(REMOTE_SERVICE_KEY, createLanAccessRemote(ctx, scope, wiring));

	// noAuth + the boot wiring report. Both wait for the web transport
	// services; the report additionally waits for the tree to settle so it
	// reads the final live bind (the same lifecycle the shipped web runtime
	// uses for its URL line). Read-only: the report logs, it never rewrites.
	ctx.inject(["connection", "webServer"], (runtimeCtx) => {
		...
		const report = () => {
			...
			wiring.connection = snapshot.trustedHosts.every((host) => configured.includes(host));
			wiring.verified = snapshot.enabled && wiring.webserver && wiring.connection;
			if (!snapshot.enabled) {
				console.log(`dsh-lan-access: LAN access disabled (webserver binds ${String(webServer.host)})`);
				return;
			}
			if (!wiring.verified) {
				console.warn(`dsh-lan-access: LAN access configured but not live (bind: ${String(webServer.host)}${webServer.host === "0.0.0.0" ? "" : ", expected 0.0.0.0"}, fence covers ${String(configured.length)} authority/authorities) — restart dsh web to apply; if it still does not take effect after the restart, this dsh version likely renamed the webserver/connection rows (see README troubleshooting)`);
				return;
			}
			...
			const primary = snapshot.accessHosts[0];
		...
		if (settle === void 0) report();
		else settle.then(report, () => {});
	});
}

...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		class LanAccessController {
			...
			async save(enabled, accessHosts, rescueSettings, sessionDays) {
				const service = this.settingsService();
				...
				await service.update(NS, { enabled, accessHosts, rescueSettings, sessionDays }, current?.revision ?? void 0);
				await this.refresh();
			}
		}
		...
		function SectionView(props) {
			...
			const onSave = async () => {
				...
				try {
					...
					await controller.save(enabled, accessHosts, rescue, days);
					dirty.current = false;
					setSaved(true);
				} catch (error) {
			...
			return React.createElement("div", { style: card }, [
				...
				React.createElement("div", { style: muted, key: "session-hint" }, t("session.hint")),
				...
				React.createElement("div", { style: muted, key: "url-hint" }, t("url.hint")),

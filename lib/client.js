...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		class LanAccessController {
			...
			async save(enabled, accessHosts, rescueSettings) {
				const service = this.settingsService();
				...
				await service.update(NS, { enabled, accessHosts, rescueSettings }, current?.revision ?? void 0);
				await this.refresh();
			}
		}
		...
		function SectionView(props) {
			...
			const onSave = async () => {
				...
				try {
					...
					await controller.save(enabled, accessHosts, rescue);
					dirty.current = false;
					setSaved(true);
				} catch (error) {

...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		class LanAccessController {
			...
			/**
			 * Keep the overview honest while the page stays open.
			 *
			 * The host process restarts on every bind change (static
			 * composition: a toggle only takes effect on the next boot), and
			 * the service events that trigger a refresh only fire on a fresh
			 * page load — a tab left open across a restart would otherwise
			 * show the dead process's snapshot (stale trusted hosts, stale
			 * "active now" badge) forever. Poll gently, and refresh right
			 * after the tab becomes visible again.
			 */
			startAutoRefresh() {
				const REFRESH_INTERVAL_MS = 30 * 1000;
				this.timer = setInterval(() => {
					if (!this.abort.signal.aborted) void this.refresh();
				}, REFRESH_INTERVAL_MS);
				const onVisible = () => {
			...
			async doRefresh() {
				// A background refresh (timer / visibility change) must not
				// flash the "reading" badge over a snapshot the page already
				// shows; only a first load starts from the loading phase.
				if (this.observable.getSnapshot()?.phase !== "ready") {
